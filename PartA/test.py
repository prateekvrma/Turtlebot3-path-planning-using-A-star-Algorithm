...
class Graph:
    ...
    def draw_action(self, i, j, theta, UL, UR, color):
        ...
        while t < 1:
            ...
            pygame.draw.line(gridDisplay, color, [int(50*oldI), int(mapHeight - 50*oldJ)], [int(50*newI), int(mapHeight - 50*newJ)], 2)
            newTheta += (r / L) * (UR - UL) * dt
    ...
    def generateGraph(self, ):
        ...
        # Circles
        pygame.draw.circle(gridDisplay, PURPLE, [100, int(mapHeight - 100)], 50)
        pygame.draw.circle(gridDisplay, PURPLE, [100, int(mapHeight - 400)], 50)

        # Rectangles
        pygame.draw.polygon(gridDisplay, PURPLE, [(int(50*0.25), int(mapHeight - 50*5.75)), (int(50*1.75), int(mapHeight - 50*5.75)), (int(50*1.75), int(mapHeight - 50*4.25)), (50*0.25, mapHeight - 50*4.25)])
        pygame.draw.polygon(gridDisplay, PURPLE, [(int(50*3.75), int(mapHeight - 50*5.75)), (int(50*6.25), int(mapHeight - 50*5.75)), (int(50*6.25), int(mapHeight) - int(50*4.25)), (int(50*3.75), int(mapHeight - 50*4.25))])
        pygame.draw.polygon(gridDisplay, PURPLE, [(int(50*7.25), int(mapHeight - 50*4)), (int(50*8.75), int(mapHeight - 50*4)), (int(50*8.75), int(mapHeight - 50*2)), (int(50*7.25), int(mapHeight - 50*2))])
    ...
    def visualizeAStar(self, start, end):
        ...
        priorityQueue = []
        heapq.heappush(priorityQueue, (start.cost, start))
        pygame.draw.circle(gridDisplay, RED, [int(50*start.i), int(mapHeight - 50*start.j)], 5)
        pygame.draw.circle(gridDisplay, RED, [int(50*end.i), int(mapHeight - 50*end.j)], 5)
        pygame.display.update()
        while len(priorityQueue):
...
## Take Input
x1 = float(input("Enter Start(X) coordinate: "))
y1 = float(input("Enter Start(Y) coordinate: "))
thetaStart = int(input("Enter Start(Theta) value: "))
print("----------------------------------------------------------")

x2 = float(input("Enter Goal(X) coordinate: "))
y2 = float(input("Enter Goal(Y) coordinate: "))
print("----------------------------------------------------------")
...
if robot.performAStar(start, end):
    ...
    gridDisplay = pygame.display.set_mode((mapWidth, mapHeight))
    pygame.display.set_caption("A-Star Visualize")
    exiting = False
    clock = pygame.time.Clock()
    grid = [[0 for j in range(mapHeight)] for i in range(mapWidth)]
...
while not exiting:
    ...
    for index in range(len(path)-1):
        ...
        robot.draw_action(node.i, node.j, node.theta, action[0], action[1], GREEN,)
